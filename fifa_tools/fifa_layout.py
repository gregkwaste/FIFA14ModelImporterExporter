...
class CrowdSection(bpy.types.Panel):
	...
	def draw(self,context):
		...
		if not context.object.name.split(sep='_')[0]=='crowd':
			row=layout.row()
			row.label(text='Not a Crowd Object, select one to activate the panel')
		else:	
			row=layout.row()
...
class Vertex_color_panel(bpy.types.Panel):
	...
	def draw(self, context):
		...
		row.template_color_picker(scn,'vx_color',value_slider=True,lock=False,lock_luminosity=False)
				...
				if op.__class__.__name__=='str':
					...
					try:
						...
						if val:
							row=actionrend_col.row()
							subbox=row.box()
							subbox.label(icon='SETTINGS',text=op[0]+' properties')
							...
							for subprop in op[sect]:
								...
								split=row.split()
								split_scale_x=0.4
								split.prop(ob.actionrender_props,subprop,text='')
					except:
						print('')
		row=layout.row()
		row.alignment='EXPAND'
...
items = [('rooffireworks','Roof Fireworks','Rooffireworks'), 
		...
		('chair_folding_blue_cushion','Folding Blue Chair','Folding Blue Chair'),
...
class emitbox_propertygroup(bpy.types.PropertyGroup):
	fAngularVelocityAdoption=FloatProperty(name='fAngularVelocityAdoption',min=0,max=100,precision=1)
	fSizeMean=FloatProperty(name='fSizeMean',min=0,max=5000,precision=1)
	fSizeSpread=FloatProperty(name='fSizeSpread',min=0,max=1000,precision=1)
...
class actionrender_propertygroup(bpy.types.PropertyGroup):
	...
	items=[('glare_corona.dds','glare_corona','glare_corona.dds'),
			('glare_lensflare_flipped','glare_lensflare_flipped','glare_lensflare_flipped.tga'),
			('glare_atmosglow.dds','glare_atmosglow','glare_atmosglow'),
			('glare_halogen.dds','glare_halogen','glare_halogen.dds'),
			('glare_punchglow.dds','glare_punchglow','glare_punchglow.dds'),
			('glare_alphamos.dds','glare_alphamos','glare_alphamos.dds'),
			('glare_half.dds','glare_half','glare_half.dds'),
			('fx_freshbeam.dds','fx_freshbeam','fx_freshbeam.dds'),
			('fx_vbeam_gradient.dds','fx_vbeam_gradient','fx_vbeam_gradient.dds')
			],
	...
	fTimeSpeed=FloatProperty(name='fTimeSpeed',min=0,max=0.5,step=0.001,default=0.001)
	iDepthBuffer=IntProperty(name='iDepthBuffer',min=0,max=5,step=1,default=1)
	iAlign=IntProperty(name='iAlign',min=0,max=5,step=1,default=0)
	iNoFadeFar=IntProperty(name='iNoFadeFar',min=0,max=5,step=1,default=0)
	iScreenSpace=IntProperty(name='iScreenSpace',min=0,max=5,step=1,default=0)
	fFadeDistance=FloatProperty(name='fFadeDistance',min=0,max=1,precision=2,step=0.01,default=0)
	fZBias=FloatProperty(name='fZBias',min=0,max=1,precision=2,step=0.01,default=0)
	
	
	fGlareSensitivityCenter=FloatProperty(name='fGlareSensitivityCenter',min=0,max=5,precision=2,step=0.01)
	fGlareSensitivityEdge=FloatProperty(name='fGlareSensitivityEdge',min=0,max=5,precision=2,step=0.01)
	fGlareSensitivityPower=FloatProperty(name='fGlareSensitivityPower',min=0,max=100,precision=2,step=0.01)
	fGlareBloomScale=FloatProperty(name='fGlareBloomScale',min=0,max=5,precision=2,step=0.01)
	fGlareBloomSpread=FloatProperty(name='fGlareBloomSpread',min=0,max=5,precision=2,step=0.01)
	fGlareBloomRate=FloatProperty(name='fGlareBloomRate',min=-1,max=1,precision=2,step=0.01)
	fGlareRotationRate=FloatProperty(name='fGlareRotationRate',min=-1,max=1,precision=2,step=0.01)
	fGlareSizeMultSpread=FloatProperty(name='fGlareSizeMultSpread',min=0,max=1,precision=2,step=0.1)
	
	fFlareMovementRate=FloatProperty(name='fFlareMovementRate',min=0,max=1,precision=4,step=0.0001)
	fFlareOffsetScale=FloatProperty(name='fFlareOffsetScale',min=0,max=1,precision=2,step=0.01)
	fFlareEndScale=FloatProperty(name='fFlareEndScale',min=0,max=20,precision=2,step=0.01)
	
	fVbeamAngle=FloatProperty(name='fVbeamAngle',min=0,max=360,precision=2,step=0.01)
	fVbeamAngleSpread=FloatProperty(name='fVbeamAngleSpread',min=0,max=20,precision=2,step=0.01)
	fVbeamLength=FloatProperty(name='fVbeamLength',min=0,max=10000,precision=2,step=1)
	fVbeamLengthSpread=FloatProperty(name='fVbeamLengthSpread',min=0,max=10000,precision=2,step=1)
	
	bUseColorRamp=BoolProperty(name='bUseColorRamp',default=False)
	vColorRamp=FloatVectorProperty(name='vColorRamp',default=(1,1,1,1),size=4,max=5,min=0,precision=1,step=0.1)
	vColorRampTimes=FloatVectorProperty(name='vColorRampTimes',default=(0,0,0,1),size=4,max=5,min=0,precision=1,step=0.1)
	
	
	bUseSizeRamp=BoolProperty(name='bUseSizeRamp',default=False)
	vSizeRamp=FloatVectorProperty(name='vSizeRamp',default=(1,1,1,1),size=4,max=5,min=0,precision=1,step=0.1)
	vSizeRampTimes=FloatVectorProperty(name='vSizeRampTimes',default=(0,0,0,1),size=4,max=5,min=0,precision=1,step=0.1)
	
	bUseSizeMult=BoolProperty(name='bUseSizeMult',default=False)
	fSizeMultX=FloatProperty(name='fSizeMultX',min=0,max=10,precision=2,step=0.01,default=4)
	fSizeMultY=FloatProperty(name='fSizeMultY',min=0,max=10,precision=2,step=0.01,default=4)
	...
	bStretchPerParticle=BoolProperty(name='bStretchPerParticle',default=False)
	
	bUseColorMult=BoolProperty(name='bUseColorMult',default=True)
	vColorMult=FloatVectorProperty(name='vColorMult',default=(1,1,1,1),size=4,max=5,min=0,precision=2,step=0.01)
	vColorMultSpread=FloatVectorProperty(name='vColorMultSpread',default=(1,1,1,1),size=4,max=5,min=0,precision=2,step=0.01)
	
	bUseGlareSeed=BoolProperty(name='bUseGlareSeed',default=True)
	
	
	
	vPivotShift=FloatVectorProperty(name='vPivotShift',default=(0,0,0),size=3,max=5,min=0,precision=2)
...
#LIGHT PROPERTIES
bpy.types.Object.emitbox_props=bpy.props.PointerProperty(type=emitbox_propertygroup)
bpy.types.Object.actionrender_props=bpy.props.PointerProperty(type=actionrender_propertygroup)
