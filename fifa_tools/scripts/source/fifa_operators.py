...
try:
    fifa_main = imp.load_source('fifa_main', prePath + os.path.join(fifa_source_path, 'fifa_main.py'))
    print('Loading Source File')
except:
    fifa_main = imp.load_compiled('fifa_main', prePath + os.path.join(fifa_compiled_path, 'fifa15_main.pyc'))
...
standard_materials = ['adboard', 'adboarddigital', 'adboarddigitalglow', 'adboarddigitalwide', 'adboardgeneric', 'adboardscrolling', 'adboardsingledigital', 'adboardsingledigitalglow', 'banneraway',
                      ...
                      'simpleglow', 'sky', 'snowpile', 'tournament', 'blendcomplex']
...
class crowd_create_seats(bpy.types.Operator):
    ...
    crowd_vertical_distance = bpy.props.FloatProperty(
        name='Vertical Distance', min=2, max=10)
    crowd_horizontal_distance = bpy.props.FloatProperty(
        name='Horizontal Distance', min=2, max=10)
    crowd_gap_distance = bpy.props.FloatProperty(
        name='Gap Distance', min=2, max=10)
    crowd_horizontal_seats = bpy.props.IntProperty(
        name='Seat Number Horizontally', min=2, max=1000)
    crowd_vertical_seats = bpy.props.IntProperty(
        name='Seat Number Vertically', min=1, max=50)
    ...
    def execute(self, context):
        ...
        fifa_main.crowd_seat_create(self.crowd_vertical_seats, self.crowd_horizontal_seats,
                                    self.crowd_vertical_distance, self.crowd_horizontal_distance, self.crowd_gap_distance, context)
...
class visit_url(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        webbrowser.open(
            url='http://www.soccergaming.com/forums/showthread.php?p=3562558')
        return{'FINISHED'}
    ...
    def invoke(self, context, event):
        ...
        for ob in scn.objects:
            if ob.name[0:7] == 'LIGHTS_':
                ...
                # EMITBOX PART
                xmlstring += indent * '\t'
                xmlstring += '<particleAction name=' + chr(34) + 'ParticleActionEmitBox' + chr(
                ...
                # ACTIONRENDER PART
                xmlstring += indent * '\t'
                xmlstring += '<particleAction name=' + chr(34) + 'ParticleActionRender' + chr(
                    34) + ' className=' + chr(34) + 'ParticleActionRender' + chr(34) + '>\n'
...
class file_import(bpy.types.Operator):
    ...
    def invoke(self, context, event):
            ...
            if scn.create_materials_flag is True:
                if not f.type.split(sep='_')[0] + '_' + str(f.id) in bpy.data.materials:
                    ...
                    new_mat = bpy.data.materials.new(
                        f.type.split(sep='_')[0] + '_' + str(f.id))
                ...
                for id in range(f.texture_count):
                    ...
                    name_fixed = name.split(
                        sep='.')[0:len(name.split(sep='.')) - 1]
        ...
        for path in paths:
            ...
            if scn.geometry_flag is True:
                ...
                for i in range(f.mesh_count):
                    ...
                    obname = fifa_main.createmesh(f.vxtable[i], f.itable[i], f.uvs[
                                                  i], f.type, objectcount, f.id, sub_name, f.cols[i], False, [], scn.fifa_import_loc)
                    objectcount += 1
                    ...
                    if scn.bone_groups_flag:
                        ...
                        for j in groups:
                            if not str(j) in bpy.data.objects[obname].vertex_groups:
                                bpy.data.objects[
                                    obname].vertex_groups.new(str(j))
                            bpy.data.objects[obname].vertex_groups[
                                str(j)].add(groups[j], 1, 'ADD')

                    # GROUP PARENTING
                for i in f.mat_assign_table:
                    bpy.data.objects[str(
                        f.type) + '_' + str(f.id) + '_' + str(i[0])].parent = bpy.data.objects['group_' + str(i[2])]
            ...
            if scn.materials_flag is True:
                for index in range(len(f.materials)):
                    ...
                    for i in f.materials[index][1]:
                        ...
                        if not i[0] in bpy.data.textures:
                            ...
                            try:
                                new_tex.image = bpy.data.images.load(
                                    os.path.realpath(i[1]))
                            except RuntimeError:
                ...
                for i in f.mat_assign_table:
                    try:
                        ...
                        bpy.data.objects['stadium_' + str(f.id) + '_' + str(i[0])].data.materials.append(
                            bpy.data.materials[f.materials[i[1]][0]])
                    except IndexError:
                        print('Index Not In Range')

            # Stadium Objects Renaming. Should be done after material
            # assignment
            if f.type == 'stadium':
                for i in range(f.mesh_count):
                    bpy.data.objects[
            ...
            if scn.bones_flag is True and len(f.bones) > 0:
                for arm_id in range(len(f.bones)):
                    ...
                    ob = bpy.data.objects.new(
                        'armature_object_' + str(arm_id), amt)
                    scn.objects.link(ob)
                    bpy.context.scene.objects.active = ob
                    bpy.ops.object.mode_set(mode='EDIT')
                    for i in range(len(f.bones[arm_id])):
                        bone = amt.edit_bones.new(
                            'mynewnewbone' + '_' + str(i))
            ...
            if scn.props_flag is True:
                ...
                for i in range(len(f.props)):
                    object_name = gh.create_prop(
                        f.props[i], f.prop_positions[i], f.prop_rotations[i])
                    ...
                    if not 'PROPS' in bpy.data.objects:
                        ...
                        bpy.data.objects[
                            object_name].parent = bpy.data.objects['PROPS']
                    else:
            ...
            if scn.collision_flag is True:
                ...
                for collision in f.collisions:
                    obname = fifa_main.createmesh(collision[1], collision[2], [], collision[
                                                  0], collisioncount, f.id, '', [], False, [], scn.fifa_import_loc)
        ...
        if not scn.obj_path == '':
            ...
            bpy.ops.import_scene.obj(filepath=scn.obj_path, axis_forward='-Z', axis_up='Y',
                                     use_image_search=True, split_mode='OFF', global_clamp_size=1, use_groups_as_vgroups=True)
        ...
        if not scn.crwd_import_path == '':
            ...
            for i in range(len(f.crowd)):
                ...
                rot_mat = Matrix.Rotation(
                    math.radians(f.crowd[i][1]) + math.radians(90.0), 4, Vector((0, 1, 0)))
            ...
            clog.close()
            crowd_name = fifa_main.createmesh(crowd_verts, crowd_faces, [
            ], f.type, 0, f.id, 'crowd', [], False, [], scn.fifa_import_loc)
            # Add the color layer
        ...
        if not scn.lnx_import_path == '':
                        ...
                        for k in range(1, len(param.childNodes), 2):
                            try:
                                ...
                                if object.attributes['name'].value == 'vCenter':
                                    ...
                                    loc = tuple(
                                        float(i) / 1000 for i in object.attributes['value'].value[1:-1].split(sep=','))
                                    bpy.ops.object.lamp_add(
                                        type='POINT', location=loc)
                                    bpy.data.objects[
                                        'Point'].name = name + '_' + index
                            except:
                                print('Skipping')
            ...
            bpy.data.objects['LIGHTS'].rotation_euler = Euler(
                (1.570796251296997, -0.0, 0.0), 'XYZ')

        return {'FINISHED'}


# TEXTURE EXPORT OPERATOR### WORKS FOR THE EXPORTER ONLY, WILL BE USED FOR
# THE OVERWRITER AS WELL
class texture_export(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        if scn.gen_overwriter_flag:
            ...
            try:
                # query the first model of the file
        ...
        if scn.face_edit_flag:
            ...
            if scn.face_edit_head_flag:
                try:
                    name = scn.model_import_path.split(
                        sep='\\')[-1].split(sep='.')[0]
                    id = name.split(sep='_')[1]
                    type = name.split(sep='_')[0]
                    head_found = False
                ...
                try:
                    ...
                    object = bpy.data.objects[
                        type + '_' + str(id) + '_0_' + 'head']
                    head_found = True
                except KeyError:
                    try:
                        # try with second mesh
                        object = bpy.data.objects[
                            type + '_' + str(id) + '_1_' + 'head']
                        head_found = True
                    except KeyError:
                ...
                if head_found:
                    ...
                    # Convert Textures to DDS
                    status = fifa_main.write_textures_to_file(
                        textures_list, type, id)
                    if status == 'missing_texture_file':
                        self.report(
                            {'ERROR'}, 'Missing ' + status.split(sep=',')[1] + ' Texture File')
                        return {'CANCELLED'}
                ...
                try:
                    # try with first mesh
                    object = bpy.data.objects[
                        type + '_' + str(id) + '_0_' + 'eyes']
                    eyes_found = True
                except KeyError:
                    try:
                        # try with second mesh
                        object = bpy.data.objects[
                            type + '_' + str(id) + '_1_' + 'eyes']
                        eyes_found = True
                    except KeyError:
            ...
            if scn.face_edit_hair_flag:
                try:
                    name = scn.hair_import_path.split(
                        sep='\\')[-1].split(sep='.')[0]
                    id = name.split(sep='_')[1]
                    type = name.split(sep='_')[0]
                except:
...
class test_file_export(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        # Calling Writing to file Functions
        if scn.stadium_export_flag:
            f = fifa_main.fifa_rx3(
                scn.export_path + 'stadium_' + str(scn.file_id) + '.rx3', 1)
        elif scn.trophy_export_flag:
            f = fifa_main.fifa_rx3(
                scn.export_path + 'trophy-ball_' + str(scn.file_id) + '.rx3', 1)
        ...
        for item in bpy.data.objects:
            ...
            if item.type == 'EMPTY' and (item.name[0:5] == 'stad_' or item.name == 'TROPHY' or item.name == 'BALL'):
                ...
                if scn.stadium_export_flag:
                    ...
                    if len(f.group_list) == 0:
                        ...
                        group_object_offset = f.group_list[-
                                                           1][4] + f.group_list[-1][3]

                    f.group_list.append(
                        [item.name, [0, 0, 0], [0, 0, 0], len(item.children), group_object_offset])
                    # Get Group Bounding Box Values

                    f.group_list[-1][1], f.group_list[-
                                                      1][2] = gh.group_bbox(item)
                ...
                for child_item in item.children:
                    ...
                    entry.colorList, entry.boundBox, entry.meshDescr, entry.meshDescrShort, entry.chunkLength = fifa_main.convert_mesh_init(
                        child_item, 0)
                    ...
                    entry.vertsCount, entry.verts, entry.uvLayerCount, entry.uvs, entry.indicesCount, entry.indices, entry.colors, entry.normals = fifa_main.convert_mesh_init(
                        child_item, 1)
                    ...
                    if scn.stadium_export_flag:
                        ...
                        try:
                            ...
                            if not mat_name in f.material_dict:
                                ...
                                # Matchup Textures with the FIFA Slot Type
                                # dictionary.
                                for i in range(10):  # 9 slots for textures
                                    try:
                                        local_texture_list.append(
                                            bpy.data.materials[mat_name].texture_slots[i].name)
                                        local_texture_name_list.append(
                                            texture_slotType_dict[i])
                                        textureCount += 1
                                    except:
                                ...
                                for i in range(textureCount):
                                    if not local_texture_list[i] in f.texture_list:
                                        f.texture_list.append(
                                            local_texture_list[i])

                                # Get object diffuse and ambient texture ids
                                try:
                                    entry_diffuse = f.texture_list.index(
                                        local_texture_list[0])
                                    # entry_ambient=f.texture_list.index(local_texture_list[1])
                                    entry_material = f.material_list.index(
                                        mat_name)
                                except:
                                    self.report(
                            ...
                            print(
                                'No material in object' + str(child_item.name))
            ...
            if scn.stadium_export_flag and item.type == 'MESH' and item.name[0:14] == 'stad_Collision':
                entry = []
                collision_tris_length, collision_verts_table, collision_part_name = fifa_main.convert_mesh_collisions(
                    item)
...
class remove_meshes(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        self.report({'INFO'}, str(temp) + ' Unused Meshes Removed ' + str(count) + ' Unused Curves Removed ' +
                    str(lcount) + ' Unused Lamps Removed ' + str(bcount) + ' Unused Armatures Removed')
        return {'FINISHED'}
            ...
            if ident in dict:
                try:
                    ...
                    obj.data.materials.append(
                        bpy.data.materials[dict[ident] + '_' + obj.name.split(sep='_')[1]])
                except KeyError:
                    self.report(
                ...
                try:
                    for j in obj.data.materials:
                        obj.data.materials.pop(0, update_data=True)
                    obj.data.materials.append(
                        bpy.data.materials[obj.name.split(sep='_')[0] + '_' + obj.name.split(sep='_')[1]])
                except KeyError:
                    self.report(
